...
- `JackCompiler.py` is the high level language compiler (High level language -> VM intermediate format).
- `VirtualMachine.py` is the virtual machine translator (VM commands -> Assembly commands).
- `Assembler.py` is the assembler and CPU simulator (Assembly commands -> binary).
